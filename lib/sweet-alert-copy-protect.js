...
// SweetCopyProtect
// 2015 (c) - Asep Maulana Ismail
// github.com/asepmaulanaismail/sweetalert-copyprotect
// built with sweetalert
;(function(window, document, undefined) {
	var _hideContext, _message, _title;
	
	var defaultParams = {
		hideContext: false,
		message: "You've tried to open context menu",
		title: "Oops..."
	};
	
	function config(params){
		if (params){
			if (params.hideContext){
				_hideContext = params.hideContext;
			}
			if (params.message){
			...
			_hideContext = defaultParams.hideContext;
			_message = defaultParams.message;
			_title = defaultParams.title;
		}
	}
	...
	function mouseDown(event)
	{
		if(event.button==2) // right click
		{
			if (getSelectedText() != ""){
				...
				swal(_title, _message, "error");
				return false;
			}else{
				if (!_hideContext)
					document.oncontextmenu=enableContext;
				else
					document.oncontextmenu=disableContext;
			}		
		}
	...
	/* 
	 * Global Variable swalcp and sweetAlertCopyProtect
	 */
	var swalcp, sweetAlertCopyProtect;
	sweetAlertCopyProtect = swalcp = function(params){
		config(params);
		init();
	}

	/*
	 * defining swalcp and sweetAlertCopyProtect function to window
	 */
    if (typeof define === 'function' && define.amd) {
      define(function() { return sweetAlertCopyProtect; });
    } else if (typeof module !== 'undefined' && module.exports) {
      module.exports = sweetAlertCopyProtect;
    } else if (typeof window !== 'undefined') {
      window.sweetAlertCopyProtect = window.swalcp = sweetAlertCopyProtect;
    }
})(window, document);